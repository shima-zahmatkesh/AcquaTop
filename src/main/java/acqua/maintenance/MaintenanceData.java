...
public class MaintenanceData {
	...
	private  TreeMap< Key , Node >  mtkn = new TreeMap< Key, Node > ( new Comparator<Key>() { //TODO  may remove the comparator
	...
	private Node mtknLastNode = null;  //  TODO may change to fisrt  key
	private int mtknSize ;  //( mtknSize = k + N)
	private int k ;
	private float minDistance = Float.MAX_VALUE;
	private int windowTime = 9;
	private int slideTime = 3;
	private int activeWindowNum = windowTime / slideTime ;
	private int currentWindow;
	private int maxWindow = 8;
	private long initialTime =1l;
	private int initialWindow = 3;
	
	
	
	public MaintenanceData(int size, int k) {
		super();
		this.mtknSize = size;
		this.k = k;
	...
	public void purgeExpiredWindow(int expiredWindow){
		...
		addToActiveWindow(expiredWindow + activeWindowNum ) ;
	//	lbp.put(expiredWindow + activeWindowNum, mtknLastNode);		//TODO   check to put in in
	}
		...
		if(lbp.containsValue(node)){
			...
			while (it.hasNext()){
				...
				if (lbp.get(key).equals(node))
					lbp.replace(key, node, newlbpNode);
				
				//update active window
				if (node.getScore() >= lbp.get(key).getScore() )
	...
	public void insertNodeToMTKN(Node newNode){
		...
		if ( fullResultForAllWindow() && ( mtknLastNode != null && newNode.getScore() < mtknLastNode.getScore()) )
			...
			//mtknPut(new Key(newNode.getObjectId() , newNode.getScore()), newNode);
...
//		if (mtkn.size() > mtknSize ){
//			removeNodeFromMTKN(mtknLastNode);
//			mtknLastNode = mtkn.get(mtkn.lastKey());
//		}
	}

	private void mtknPut(Key key, Node newNode) {
		if (mtknContainsNode(newNode)){
			mtkn.put(new Key(newNode.getObjectId() , newNode.getScore()), newNode);
		}
		else{
			mtkn.remove(new Key(newNode.getObjectId() , newNode.getScore()));
			mtkn.put(new Key(newNode.getObjectId() , newNode.getScore()), newNode);
		}
	...
	private void moveLBP(Key mtknKey){
	...
	private  Node  findTopkEntryofWindow(int window) {
	...
	public void outputTopKResult() {
		...
		int index = 0;
		Iterator<Key> it = mtkn.keySet().iterator();
		System.out.println("Top-k results:" + currentWindow);
	...
	public void printMTKN() {
		...
		Iterator<Key> it = mtkn.keySet().iterator();
		System.out.println("mtkn results:");
		while (it.hasNext()){
			Key key = it.next();
			System.out.println("[" + key.getObjectId() + " , score=" + key.getScore() + " , sw=" + mtkn.get(key).getStartWin() + " , ew=" + mtkn.get(key).getEndWin() + " , t=" + mtkn.get(key).getTime() +"]");
	...
	public void updateMtkn(Node newNode) {
		...
		//System.out.println(mtkn.entrySet());
		if ( mtknContainsNode(newNode)){
	...
	public void addToActiveWindow(int i){
		activeWindow.put(i, 0);	
	}
	...
	public void initializeMtkn(){
		...
		Node node1 = new Node(99l,800f,1,2,9l);
		mtkn.put(new Key(node1.getObjectId() , node1.getScore()), node1);
		
		Node node2 = new Node(97l,800f,1,2,7l);
		mtkn.put(new Key(node2.getObjectId() , node2.getScore()), node2);
		
		Node node3 = new Node(94l,700f,1,1,4l);
		mtkn.put(new Key(node3.getObjectId() , node3.getScore()), node3);
		
		Node node4 = new Node(98l,600f,2,2, 8l);
		mtkn.put(new Key(node4.getObjectId() , node4.getScore()), node4);
		...
		activeWindow.put(1,3);
		activeWindow.put(2,3);
		activeWindow.put(3,0);
	...
	private long endOfWindow(long window) {
		long time = (int) (getInitialTime()+ (window * getSlideTime()) + getWindowTime()) ;
		return time;
	}
