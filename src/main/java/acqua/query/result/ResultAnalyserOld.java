...
public class ResultAnalyserOld {
...
//---------------------------------------------------------------------fill baseline table
			...
			//---------------------------------------------------------------------fill WBMFJ
			fis = new FileInputStream(Config.INSTANCE.getProjectPath()+Config.INSTANCE.getDatasetFolder()+"joinOutput/WBMFOperatorSSOutput.txt");
			br = new BufferedReader(new InputStreamReader(fis));
			line=null;
			while((line=br.readLine())!=null)
			{
...
//Functions related to Top-k Aqua
	...
	public static void analysisExperimentJaccard(){
		...
		try{
			...
			Double cOC=0.0, cdwe=0.0, cre=0.0,csse=0.0, clrue=0.0, cfe=0.0, cspe=0.0 ,clrufe =0.0 ,crndfe =0.0 ,cwbmfe =0.0;
	...
	public static void analysisMultipleExperimentsJaccard(int index ){
		...
		try{
			...
			bw.write("timestampe,Oracle Min,WST Min,RND Min,WBM Min,LRU Min,Filter Min,LRU.F Min,RND.F Min,WBM.F Min,"
					+ "Oracle Max,WST Min,RND Max,WBM Max,LRU Max,Filter Max,LRU.F Max,RND.F Max,WBM.F Max,"
					+ "Oracle Avg,WST Avg,RND Avg,WBM Avg,LRU Avg,Filter Avg,LRU.F Avg,RND.F Avg,WBM.F Avg\n");
			
			
			//Min variables
			Double OCMin = Double.MAX_VALUE, dweMin = Double.MAX_VALUE, reMin = Double.MAX_VALUE, sseMin = Double.MAX_VALUE, lrueMin = Double.MAX_VALUE, speMin = Double.MAX_VALUE, feMin = Double.MAX_VALUE,lrufeMin =Double.MAX_VALUE ,rndfeMin =Double.MAX_VALUE ,wbmfeMin =Double.MAX_VALUE;
			//Max variables
			Double OCMax = 0.0, dweMax= 0.0, reMax = 0.0,sseMax = 0.0, lrueMax = 0.0, feMax = 0.0,lrufeMax =0.0 ,rndfeMax =0.0 ,wbmfeMax =0.0;
			//Average variables 
			Double OCAvg = 0.0, dweAvg = 0.0, reAvg = 0.0, sseAvg = 0.0, lrueAvg = 0.0, feAvg = 0.0,lrufeAvg =0.0 ,rndfeAvg =0.0 ,wbmfeAvg =0.0;
			//Sum variables 
			Double OCSum = 0.0, dweSum = 0.0, reSum = 0.0, sseSum = 0.0, lrueSum = 0.0, feSum = 0.0,lrufeSum =0.0 ,rndfeSum =0.0 ,wbmfeSum =0.0;
			long nextTime = 0;
			...
			for ( int e = 0 ; e <= Config.INSTANCE.getExperimentIterationNumber() ; e++){
				...
				bw.write(nextTime+","+OCMin+","+
						(dweMin==null?0:dweMin)+","+
						(reMin==null?0:reMin)+","+
						(sseMin==null?0:sseMin)+"," +
						(lrueMin==null?0:lrueMin)+","+
						//(speMin==null?0:speMin)+","+
						(feMin==null?0:feMin)+","+
						(lrufeMin==null?0:lrufeMin)+","+
						(rndfeMin==null?0:rndfeMin)+","+
						(wbmfeMin==null?0:wbmfeMin)+","+
						OCMax+","+
						(dweMax==null?0:dweMax)+","+
						(reMax==null?0:reMax)+","+
						(sseMax==null?0:sseMax)+"," +
						(lrueMax==null?0:lrueMax)+","+
						//(speMax==null?0:speMax)+","+
						(feMax==null?0:feMax)+","+
				...
				OCMin = Double.MAX_VALUE; dweMin = Double.MAX_VALUE; reMin = Double.MAX_VALUE; sseMin = Double.MAX_VALUE; lrueMin = Double.MAX_VALUE; speMin =Double.MAX_VALUE;  feMin = Double.MAX_VALUE;lrufeMin =Double.MAX_VALUE ;rndfeMin =Double.MAX_VALUE ;wbmfeMin =Double.MAX_VALUE;
				OCMax = 0.0; dweMax = 0.0; reMax = 0.0; sseMax = 0.0; lrueMax = 0.0; feMax = 0.0; lrufeMax =0.0; rndfeMax =0.0; wbmfeMax =0.0;
				OCAvg = 0.0; dweAvg = 0.0; reAvg = 0.0; sseAvg = 0.0; lrueAvg = 0.0; feAvg = 0.0; lrufeAvg =0.0; rndfeAvg =0.0; wbmfeAvg =0.0;
				OCSum = 0.0; dweSum = 0.0; reSum = 0.0; sseSum = 0.0; lrueSum = 0.0; feSum = 0.0; lrufeSum =0.0; rndfeSum =0.0; wbmfeSum =0.0;
			}
			bw.close();
		}catch(Exception e){e.printStackTrace();}
